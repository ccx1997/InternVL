...
def preprocess_vlnce(
    ...
    template_name = 'internvl3_5_gpt_oss'
    ...
    ## TODO: 这里应该后面要改多轮对话，把navila里面LazyVLNCEDataset的任务模版改进去
    image_tokens = "<image>\n" * (num_image - 1)
    question_template = (
    ...
    if not text_only:
        ...
        for conversation in conversations:
            if conversation['from'] == 'human':
                # 填入模版
                conversation['value'] = question_template.format(
                history_images=image_tokens,
...
def preprocess_internvl3_5_gpt_oss(
    ...
    for conversation in conversations:
        if conversation['from'] == 'human':
            ...
            batches.append(f'<|start|>assistant<|message|>{conversation["value"]}<|end|>')
            roles.append('gpt')
