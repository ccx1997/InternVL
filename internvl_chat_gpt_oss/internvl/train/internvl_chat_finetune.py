...
# from internvl_chat_gpt_oss.internvl.train.internvl_chat_finetune import use_tcs_loader
try:
...
from internvl.train.dataset import (
    ConcatDataset, TCSLoader, preprocess_vlnce,
...
class LazySupervisedDataset(Dataset):
    ...
    def video_get_item(self, data_item):
        ...
        special_tokens = '\n'.join(['Clip-{}: <image>'.format(i + 1) for i in range(len(image_list))])
        ...
        # 这里是抽取46张图像，每个图像256个token
        num_image_tokens = [self.num_image_token] * num_patches
        num_image_for_preprocess = num_patches
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  self.tokenizer, num_image_tokens, group_by_length=self.group_by_length,
                                  ds_name=self.ds_name, num_image=num_image_for_preprocess)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        # TODO：有多个0的id
        # logger.info(f"__getitem__ called with i={i}, dataset={self.ds_name}")
...
class LazyVLNCEDataset(Dataset):
    """Dataset for vln-ce fine-tuning."""
    ...
    def __init__(
        ...
        self.min_num_frame = min_num_frame
        self.sampling_method = sampling_method
        self.max_observations = self.max_num_frame
    ...
    def get_preprocess_function(self, use_pretrain=False):
        ...
        if self.template_name == 'VLNCE':
            return preprocess_vlnce
