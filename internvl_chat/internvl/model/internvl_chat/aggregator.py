...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> Tuple[List[torch.Tensor], int]:
        """
        Args:
            images (torch.Tensor): Input images with shape [B, S, 3, H, W], in range [0, 1].
                B: batch size, S: sequence length, 3: RGB channels, H: height, W: width
            attention_mask (torch.Tensor, optional): Mask for padded frames.
                Shape [B, S], 1 for valid frames, 0 for padding. Defaults to None.
        Returns:
    ...
    def _process_frame_attention(self, tokens, B, S, P, C, frame_idx, pos=None, attention_mask=None):
        ...
        Args:
            ...
            frame_idx (int): Current frame block index.
        ...
        for _ in range(self.aa_block_size):
            ...
            if self.training:
                ...
                # Note: Checkpointing with attention mask is not directly supported,
                # so we only apply it when no mask is provided.
