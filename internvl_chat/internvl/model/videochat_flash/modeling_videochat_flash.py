...
'''
default prompt be like:
<|im_start|>system
You are a helpful assistant.<|im_end|>
<|im_start|>user
<image>
The video lasts for 3.60 seconds, and 64 frames are uniformly sampled from it. Describe this video in detail.<|im_end|>
<|im_start|>assistant
The video takes place in a sunny suburban street with houses, palm trees, and parked cars. Two shirtless men are engaged in an altercation; one is wearing black shorts while the other wears dark pants. The man in black shorts appears to be attacking or defending himself against the other individual who seems to be attempting to restrain him. There's visible tension as they grapple on the pavement. A white car is parked nearby, and there's a trash bin next to it. The environment suggests this could be a residential area during daytime due to the bright sunlight casting shadows on the ground.<|im_end|>
<|im_start|>user
...
class VideoChatFlashQwenModel(LlavaMetaModel, Qwen2Model_Flash):
    ...
    def _gradient_checkpointing_func(self, func, *args, **kwargs):
        """
        为兼容transformers框架添加的gradient checkpointing函数
        当transformers框架调用gradient checkpointing时会使用这个函数
        """
...
class VideoChatFlashQwenForCausalLM(LlavaMetaForCausalLM, Qwen2ForCausalLM_Flash):
    ...
    def __init__(self, config, latent_len=1024):
        ...
        # Add $latent_len$ latent embeddings
        self.latent_embeddings = nn.Parameter(torch.randn(latent_len, config.hidden_size) * config.hidden_size ** -0.5)
    ...
    def _set_gradient_checkpointing(self, module=None, value=False):
        """设置gradient checkpointing"""
        if module is None:
            module = self.model
            
        # 为主模型设置gradient checkpointing
        if hasattr(module, "gradient_checkpointing"):
            module.gradient_checkpointing = value
            
        # 如果模型有layers，为每一层设置gradient checkpointing
        if hasattr(module, "layers"):
            for layer in module.layers:
                if hasattr(layer, "gradient_checkpointing"):
                    layer.gradient_checkpointing = value
        
        # 设置vision tower的gradient checkpointing
        vision_tower = self.get_vision_tower()
        if vision_tower is not None:
            if hasattr(vision_tower, "gradient_checkpointing"):
                vision_tower.gradient_checkpointing = value
            # 如果vision tower有encoder，也设置其gradient checkpointing
            if hasattr(vision_tower, "encoder") and hasattr(vision_tower.encoder, "gradient_checkpointing"):
                vision_tower.encoder.gradient_checkpointing = value
                
        # 如果有_gradient_checkpointing_func方法，也调用它（兼容性）
        if hasattr(self, "_gradient_checkpointing_func"):
            try:
                self._gradient_checkpointing_func(module, value)
            except Exception as e:
                # 如果标准方法失败，继续使用我们的实现
                pass
    ...
    def enable_gradient_checkpointing(self):
        ...
        # 确保设置了标准的transformers gradient checkpointing属性
    ...
    def disable_gradient_checkpointing(self):
        ...
        # 确保禁用了标准的transformers gradient checkpointing属性
        if hasattr(self, 'gradient_checkpointing'):
    ...
    def set_attention_implementation(self, attn_implementation: str = "flash_attention_2"):
        """
        设置attention实现类型
        Args:
            attn_implementation: 可选值 ["eager", "flash_attention_2", "sdpa"]
        """
        valid_implementations = ["eager", "flash_attention_2", "sdpa"]
        if attn_implementation not in valid_implementations:
            raise ValueError(f"attn_implementation must be one of {valid_implementations}, got {attn_implementation}")
        
        # 检查flash_attention_2是否可用
        if attn_implementation == "flash_attention_2":
            try:
                from transformers.utils import is_flash_attn_2_available
                if not is_flash_attn_2_available():
                    print("Warning: flash_attention_2 is not available, falling back to sdpa")
    ...
    def enable_flash_attention_2(self):
        """启用Flash Attention 2"""
        self.set_attention_implementation("flash_attention_2")
    ...
    def enable_inference_optimizations(self):
        ...
        self.config.output_hidden_states = True  # 需要hidden states来提取latent features
        
        print("✅ 推理优化已启用")
        print("  - Flash Attention 2: 启用")
        print("  - 模式: 评估模式")
        print("  - 输出优化: 仅保留必要的hidden states")
    ...
    def enable_training_optimizations(self):
        ...
        # 优化配置
        self.config.output_attentions = False  # 训练时通常不需要
        self.config.output_hidden_states = True  # 需要用于提取latent features
        
        print("✅ 训练优化已启用")
        print("  - Flash Attention 2: 启用")
        print("  - Gradient Checkpointing: 启用")
        print("  - 模式: 训练模式")
        print("  - 内存优化: 已配置")

    @torch.no_grad()
    def extract_features_batch(self, video_batch, batch_size=4):
        """
        批量提取视频特征，内存优化版本
        Args:
            video_batch: 视频批次
        ...
        for i in range(0, len(video_batch), batch_size):
            batch = video_batch[i:i+batch_size]
            features = self.forward(
                images=batch,
                modalities=["video"] * len(batch),
                use_cache=False,
                output_attentions=False,
                output_hidden_states=True
            )
            all_features.append(features[0].cpu())  # 移到CPU节省GPU内存
    ...
    def forward(
        ...
        assert labels is None, "labels is not None"
        self.config.tokenizer_padding_side = "left"
        if inputs_embeds is None:
            (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds, labels) = self.prepare_inputs_labels_for_multimodal(input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities, image_sizes)
        assert inputs_embeds is not None, "inputs_embeds is None"

        # Add latent embeddings
        latent_embeddings = self.latent_embeddings.unsqueeze(0).repeat(inputs_embeds.shape[0], 1, 1)
        inputs_embeds = torch.cat([inputs_embeds, latent_embeddings], dim=1)
        ...
        latent_features = outputs.hidden_states[:, -self.latent_len:, :] # [batch_size, latent_len, hidden_size]
