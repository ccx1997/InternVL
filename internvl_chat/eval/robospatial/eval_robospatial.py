...
def evaluate_answer(ground_truth, generated_answer):
    ...
    Returns a tuple of (is_correct, is_binary_answer, parsed_answer, is_parsable, point_score).
    point_score is the ratio of correct points to total points for polygon tasks.
    """
    ...
    if gt_lower in ["yes", "no"]:
        ...
        point_score = 1.0 if correct else 0.0
        return correct, is_binary, gen_answer, is_parsable, point_score
    else:
        ...
        try:
            ...
            if not isinstance(gt_polygon, list) or len(gt_polygon) < 3:
                return False, is_binary, parsed_answer, is_parsable, point_score
            
            # Try to match all tuple formats (x,y) or (x, y)
            tuple_matches = re.findall(r'\(\s*(\d+\.?\d*)\s*,\s*(\d+\.?\d*)\s*\)', generated_answer)
            if tuple_matches:
                try:
                    valid_points = []
                    correct_points = 0
                    ...
                    for match in tuple_matches:
                        ...
                        if point_in_polygon(x, y, gt_polygon):
                            correct_points += 1
                    
                    if len(valid_points) > 0:
                        parsed_answer = valid_points
                        is_parsable = True
                        correct = correct_points > 0
                        point_score = correct_points / len(valid_points)
                        return correct, is_binary, parsed_answer, is_parsable, point_score
                except (ValueError, TypeError):
                    pass  # Continue to other formats if float conversion fails
            
            # Try to match list format [x,y] or [x, y] - but this is less common for multiple points
            ...
            try:
                ...
                try:
                    ...
                    # If direct parsing fails, try to extract all tuples from the content
                    tuple_matches = re.findall(r'\(\s*(\d+\.?\d*)\s*,\s*(\d+\.?\d*)\s*\)', list_content)
                    if tuple_matches:
                        try:
                            valid_points = []
                            correct_points = 0
                ...
                if isinstance(gen_val, list):
                    ...
                    if len(gen_val) == 2 and all(isinstance(v, (int, float)) for v in gen_val):
                        gen_points = [tuple(gen_val)]  # Convert [x, y] to [(x, y)]
                    ...
                    # Case 3: The list contains coordinate pairs as lists [[x, y], ...]
                        ...
                        gen_points = [tuple(point) for point in gen_val]  # Convert [[x, y], ...] to [(x, y), ...]
                    else:
                        return False, is_binary, parsed_answer, is_parsable, point_score
                ...
                # Validate and evaluate all points
                valid_points = []
                correct_points = 0
                ...
                # Consider correct if at least one point is inside the polygon
                # You might want to adjust this criterion based on your requirements
                parsed_answer = valid_points
                is_parsable = True
                correct = correct_points > 0
                point_score = correct_points / len(valid_points)
...
def eval_pregenerated_results(gt_data, results_data, data_dir):
    ...
    for idx, gt_entry in enumerate(tqdm(gt_data, desc="Evaluating Pre-generated Results")):
        ...
        # Evaluate the answer
        correct, is_binary, parsed_answer, is_parsable, point_score = evaluate_answer(ground_truth, generated_answer)
        ...
        results.append({
            ...
            "num_parsed_points": len(parsed_answer) if parsed_answer is not None else 0,
            "correct": correct,
...
def main():
    ...
    results_file = "/mnt/chensenda/codes/VLN/InternVL/internvl_chat/eval/robospatial/robospatial_predictions_2.4_9600.json"
