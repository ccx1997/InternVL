...
def process_question(model, tokenizer, video_path, question, base_path="/mnt/chensenda/codes/VLN/SQA3D/assets/data/sqa_task/balanced"):
...
def main():
    # Load test data
    test_file = "/mnt/chensenda/codes/VLN/ScanQA/ScanQA_v1.0_val_reformat_std_video.json"
    ...
    for i, sample in enumerate(tqdm(test_data, desc="Processing samples")):
        ...
        if human_question is None:
            print(f"Warning: No human question found for sample {i}")
            # Add empty InternVL response
            sample['conversations'].append({
                "from": "internVL",
                "value": "Error: No human question found"
            })
            continue
    ...
    # Save results back to the same format
    output_file = "/mnt/chensenda/codes/VLN/InternVL/internvl_chat/scanqa_predictions_internvl3_8b.json"
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(test_data, f, indent=2, ensure_ascii=False)
    
    print(f"Results saved to {output_file}")
    print(f"Total processed: {len(test_data)} samples")
